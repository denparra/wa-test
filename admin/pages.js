    ...
    const content = `<section class="panel">
      ...
      ${renderTable({
        columns: [
            ...
            { key: 'message_body', label: 'Mensaje', render: (row) => escapeHtml(truncate(row.message_body || '', 60)) },
...
export function renderCampaignDetailPage({ campaign, recipients, offset, limit }) {
    const header = `<section class="panel">
      ...
      <div class="muted">Mensaje: ${escapeHtml(campaign.message_body || 'n/a')}</div>
    </section>`;
