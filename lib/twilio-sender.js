...
export async function sendOneRecipient({
    ...
    const contact = recipient.vehicle_id
        ? getContactByVehicle(recipient.vehicle_id)
        : getContactWithVehicle(recipient.contact_id);
    ...
    if (recipient.vehicle_id && isVehicleSuppressed(recipient.vehicle_id)) {
        updateCampaignRecipientStatus({
            id: recipient.id,
            status: 'skipped_vehicle_suppressed',
            errorMessage: 'vehicle_suppressed'
        });
        return { outcome: 'skipped', reason: 'vehicle_suppressed' };
    }

    const variables = buildTemplateVariables(contact || {});
    const isTwilioTemplateCampaign = campaign.type === 'twilio_template';
    ...
    const outboundBody = rendered || (isTwilioTemplateCampaign ? '[Plantilla de WhatsApp enviada]' : '');

    if (isTwilioTemplateCampaign) {
        if (!contentSid) {
            updateCampaignRecipientStatus({
                id: recipient.id,
                status: 'failed',
                errorMessage: 'missing_content_sid'
            });
            return { outcome: 'failed', reason: 'missing_content_sid' };
        }
    } else if (!rendered) {
        updateCampaignRecipientStatus({
            id: recipient.id,
